...
$('#formVenta').addEventListener('submit', async e => { e.preventDefault(); const d=valor(e.currentTarget); try { await api('/api/ventas',{method:'POST',body:JSON.stringify(d)}); const v=vehiculos.find(x=>String(x.id_vehiculo)===d.idVehiculo), comprador=clientes.find(x=>String(x.id_cliente)===d.idComprador), vendedor=clientes.find(x=>String(x.id_cliente)===String(v.id_vendedor)); $('#acta').hidden=false;$('#acta').innerHTML=`<h3>Acta de compraventa</h3><p>Fecha: ${new Date().toLocaleDateString('es-MX')}</p><p><strong>Vehículo:</strong> ${escapar(v.marca)} ${escapar(v.linea)} ${v.modelo}, serie ${escapar(v.numero_serie)}.</p><p><strong>Vendedor:</strong> ${escapar(vendedor.nombre_completo)}.</p><p><strong>Comprador:</strong> ${escapar(comprador.nombre_completo)}.</p><p><strong>Precio final:</strong> ${moneda.format(d.precioFinal)}.</p><button class="boton" onclick="window.print()">Imprimir acta</button>`;e.currentTarget.reset();await cargarTodo();aviso('Venta registrada.'); } catch(x) { aviso(x.message); } });
